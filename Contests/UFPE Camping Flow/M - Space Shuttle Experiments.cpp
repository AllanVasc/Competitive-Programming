...
int32_t main(){
    ...
    while(t--){
        ...
        vector<int> instrument_price;
        instrument_price.resize(n);
        for(int i = 0; i < n; i++) cin >> instrument_price[i];

        vector<vector<int>> requirements;
        requirements.resize(m);
        for(int i = 0; i < m; i++){
            int qtd;
            cin >> qtd;
            for(int j = 0; j < qtd; j++){
                int aux;
                cin >> aux;
                requirements[i].push_back(aux);
            }
        }

        Dinic g(m+n+2,0,m+n+1);

        for(int i = 1; i <= m; i++) g.addEdge(0,i,wins[i-1]);
        for(int i = 0; i < m; i++){
            for(int j = 0; j < requirements[i].size(); j++){
                g.addEdge(i+1,m+requirements[i][j], INF);
            }
        }
        for(int i = 1; i <= n; i++) g.addEdge(m+i,n+m+1,instrument_price[i-1]);

        cout << "Case " << count << ": " << total - g.run() << '\n';
        count++;
    }
    return 0;
}
