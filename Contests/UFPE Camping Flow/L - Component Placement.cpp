...
int32_t main(){
    ...
    while(t--){
        ...
        vector<int> price_top(n);
        for(int i = 0; i < n; i++) cin >> price_top[i];
        vector<int> price_bottom(n);
        for(int i = 0; i < n; i++) cin >> price_bottom[i];
        vector<int> layer(n);
        for(int i = 0; i < n; i++) cin >> layer[i];

        vector<ii> connexion(m);
        map<ii,int> prices_connexion;
        ...
        for(int i = 0; i < m; i++){
            ...
            if(f > s) swap(f,s);
            connexion[i].first = f;
            connexion[i].second = s;
            prices_connexion[{f,s}] = r;
        }
        ...
        for(int i = 0; i < n; i++){
            if(layer[i] == 1){
                g.addEdge(0,i+1,price_top[i]);
                g.addEdge(i+1,n+1,INF);
            }
            else if(layer[i] == -1){
                g.addEdge(0,i+1,INF);
                g.addEdge(i+1,n+1,price_bottom[i]);
            }
            else{
                g.addEdge(0,i+1,price_top[i]);
                g.addEdge(i+1,n+1,price_bottom[i]);
        ...
        for(int i = 0; i < m; i++){
            int f = connexion[i].first;
            int s = connexion[i].second;
            g.addEdge(f,s, prices_connexion[{f,s}]);
            g.addEdge(s,f, prices_connexion[{f,s}]);
        }

        cout << "Case " << count << ": " << g.run() << '\n';
        count++;
    }
    return 0;
}
