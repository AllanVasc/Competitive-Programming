...
int32_t main(){
    ...
    vector<int> red_cost;
    vector<int> green_cost;
    red_cost.resize(n);
    green_cost.resize(n);
    for(int i = 0; i < n; i++) cin >> red_cost[i];
    for(int i = 0; i < n; i++) cin >> green_cost[i];

    Dinic g(2*n+2,0,2*n+1);

    for(int i = 1; i<=n;i++) g.addEdge(0,i,red_cost[i-1]);
    for(int i = 1; i<=n; i++) g.addEdge(n+i, 2*n+1, green_cost[i-1]);
