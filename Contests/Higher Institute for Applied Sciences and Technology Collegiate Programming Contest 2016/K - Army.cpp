...
int32_t main(){
    ...
    while(t--){
        ...
        vector<set<int>> prefer_places;
        prefer_places.resize(n);
        for(int i = 0; i < n; i++){
            int qtd;
            cin >> qtd;
            for(int j = 0; j < qtd; j++){
                int aux;
                cin >> aux;
                prefer_places[i].insert(aux);
            }
        }

        vector<set<int>> prefer_weapons;
        prefer_weapons.resize(n);
        for(int i = 0; i < n; i++){
            int qtd;
            cin >> qtd;
            for(int j = 0; j < qtd; j++){
                int aux;
                cin >> aux;
                prefer_weapons[i].insert(aux);
            }
        }

        vector<set<int>> weapons_in_place;
        weapons_in_place.resize(m);
        for(int i = 0; i < m; i++){
            int qtd;
            cin >> qtd;
            for(int j = 0; j < qtd; j++){
        ...
        for(int i = 0; i < n ;i++){
            for(auto j: prefer_places[i]){
                for(auto k : prefer_weapons[i]){
                    if(weapons_in_place[j-1].find(k) != weapons_in_place[j-1].end()){
                        g.addEdge(i+1,n+j,1);
                    }
                }
            }
